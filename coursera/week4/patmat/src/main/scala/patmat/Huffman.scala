...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def tuplify(chars: List[Char]): List[(Char, Int)] = chars match {
      case Nil => List()
      case x :: xs => List((x, 1)) ::: tuplify(xs)
    }
    tuplify(chars).groupBy(_._1).mapValues(_.map(_._2).sum).toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.map(t => Leaf(t._1, t._2)).sortWith(_.weight < _.weight)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def _internal(remainingTree: CodeTree, bits: List[Bit]): List[Char] = {
      (remainingTree, bits) match {
        case (Leaf(c, _), Nil) => List(c)
        case (Leaf(c, _), xb) => c :: _internal(tree, xb)
        case (Fork(left, right, _, _), 1 :: xb) => _internal(right, xb)
        case (Fork(left, right, _, _), 0 :: xb) => _internal(left, xb)
      }
    }
    _internal(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def _internal(remainingTree: CodeTree, text: List[Char]): List[Bit] = {
      (remainingTree, text) match {
        case (_, Nil) => List()
        case (Leaf(_, _), c :: xb) => _internal(tree, xb)
        case (Fork(left, right, _, _), xb) if chars(left).contains(xb.head) => 0 :: _internal(left, xb)
        case (Fork(left, right, _, _), xb) => 1 :: _internal(right, xb)
      }
    }
    _internal(tree, text)
  }
